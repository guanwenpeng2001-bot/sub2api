...
func TestGPTImage25StaticPricingResource(t *testing.T) {
...
func TestGPTImage25UsageBoundsImageInputTokens(t *testing.T) {
	for _, tc := range []struct {
		...
		{"missing", `{}`, 0},
		{"negative", `{"image_tokens":-1}`, 0},
		{"exceeds total", `{"image_tokens":2000}`, 1550},
	} {
		t.Run(tc.name, func(t *testing.T) {
			var usage OpenAIUsage
			svc := &OpenAIGatewayService{}
			body := fmt.Sprintf(`{"type":"response.completed","response":{"tool_usage":{"image_gen":{"input_tokens":1550,"input_tokens_details":%s,"output_tokens":515,"output_tokens_details":{"image_tokens":515}}}}}`, tc.details)
			svc.parseOpenAIImagesSSEUsageBytes([]byte(body), &usage)
			require.Equal(t, 1550, usage.InputTokens)
