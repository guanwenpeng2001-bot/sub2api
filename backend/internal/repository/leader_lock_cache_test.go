...
func TestLeaderLockCache_RenewOnlyCurrentOwner(t *testing.T) {
	cache, mr := newLeaderLockTestCache(t)
	ctx := context.Background()
	ok, err := cache.TryAcquireLeaderLock(ctx, "sync", "first", time.Minute)
	require.NoError(t, err)
	require.True(t, ok)
	mr.FastForward(40 * time.Second)
	ok, err = cache.RenewLeaderLock(ctx, "sync", "stale", time.Minute)
	require.NoError(t, err)
	require.False(t, ok)
	ok, err = cache.RenewLeaderLock(ctx, "sync", "first", time.Minute)
	require.NoError(t, err)
	require.True(t, ok)
	mr.FastForward(40 * time.Second)
	ok, err = cache.TryAcquireLeaderLock(ctx, "sync", "second", time.Minute)
	require.NoError(t, err)
	require.False(t, ok)
	...
	require.NoError(t, err)
	require.False(t, ok)
	require.NoError(t, cache.ReleaseLeaderLock(ctx, "sync", "first"))
	ok, err = cache.RenewLeaderLock(ctx, "sync", "second", time.Minute)
	require.NoError(t, err)
	require.True(t, ok)
}
