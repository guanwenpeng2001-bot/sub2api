...
OAuth / Setup Token 图片请求使用 Responses 主控模型调用 `image_generation` 工具，默认主控为 `gpt-5.6-luna`。部署模板不预设此值，留空仍使用上述代码回退值；上线前须由部署侧设置 `SUB2API_IMAGES_MAIN_MODEL` 并验证账号支持的文本模型；Docker Compose 用户修改 `.env` 后执行 `docker compose up -d` 重建容器。该配置不会替换所选图片模型，也不会覆盖 `/v1/responses` 请求中已经提供的文本主控模型。
